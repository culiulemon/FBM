...
export function fuzzyMatchTitle(normalized, target, maxDistance = 2) {
...
export function parseMarkdown(content, filePath) {
    ...
    function flushParagraph(parent) {
    ...
    function flushCodeBlock(parent) {
        if (!currentCodeBlock)
            return;
        const node = {
            type: 'code',
...
export class NodeLocator {
    locateByHeadingPath(headings, path) {
        if (path.length === 0)
            return null;
        return this.findInTree(headings, path, 0);
    }
    findInTree(headings, path, depth) {
        ...
        for (const h of headings) {
            if (fuzzyMatchTitle(h.normalizedTitle, normalizedTarget)) {
                if (depth === path.length - 1)
                    return h;
                const childHeadings = h.children.filter((n) => n.type === 'heading');
    ...
    locateCodeBlocks(headings, options) {
        const blocks = [];
    ...
    collectCodeBlocks(nodes, result, headingFilter) {
    ...
    async searchFiles(rootDir, pattern) {
        const results = [];
        await this.scanDir(rootDir, results, pattern);
        return results;
    }
    async scanDir(dir, results, pattern) {
        let entries;
        try {
