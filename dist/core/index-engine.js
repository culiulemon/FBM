...
export class IndexEngine {
    ...
    memoryDir;
    cacheFile;
    constructor(memoryDir, cacheFile) {
    ...
    async indexFile(filePath) {
        try {
            ...
            for (const heading of headings) {
                ...
                const headingIndex = { filePath, heading, nodeRef };
    ...
    createNodeRef(heading, filePath, mtimeMs) {
    ...
    buildHeadingPath(heading) {
        ...
        while (current.children) {
            const childHeading = current.children.find((n) => n.type === 'heading');
            if (!childHeading)
                break;
    ...
    indexKeywords(node, ref) {
        ...
        for (const token of tokens) {
            if (!this.index.keywords[token]) {
                ...
                const exists = entry.refs.some(r => r.filePath === ref.filePath && r.headingPath.join('/') === ref.headingPath.join('/'));
                if (!exists) {
                    entry.refs.push(ref);
                }
                entry.frequency++;
    ...
    removeNodeRefKeywords(ref) {
        const refKey = `${ref.filePath}:${ref.headingPath.join('/')}`;
        for (const [token, entry] of Object.entries(this.index.keywords)) {
            entry.refs = entry.refs.filter(r => !(r.filePath === ref.filePath && r.headingPath.join('/') === refKey.split(':').slice(1).join('/')));
    ...
    async loadCache() {
        ...
        try {
            ...
            for (const [filePath, indices] of Object.entries(serialized.files)) {
                const headingIndices = indices.map(si => ({
                    filePath: si.filePath,
                    heading: {
                        type: 'heading',
                        level: si.heading.level,
